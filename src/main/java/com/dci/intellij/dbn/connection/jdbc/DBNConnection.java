...
public class DBNConnection extends DBNConnectionBase {
    ...
    private
    PendingTransactionBundle dataChanges;
    private
    SchemaId currentSchema;

    private
    final Set<DBNStatement> activeStatements = new HashSet<>();
    ...
    public void statusChanged(ResourceStatus status) {
        ...
        if (Failsafe.check(connectionHandler)) {
            ...
            switch (status) {
                case CLOSED:
                    connectionStatus.getConnected().markDirty();
                    break;
                case VALID:
                    connectionStatus.getValid().markDirty();
                    break;
                case ACTIVE:
                    connectionStatus.getActive().markDirty();
                    break;
            }
        }
    }
